...
`C++` code in the LibTorch library. This code is
...
library(torchvision) # for datasets we are going to use
...
We have create a model called `modnn` by defining the `initialize` and `forward` functions and passing them to the `nn_module()` function. The `initialize` function is responsible for initializing the submodules that are used by the model. In the `forward` method we implement what happens when call that model with observations. In this case we use the layers we defined in `initialize` in that specific order.

`self` is a list-like special object that is is used to share information between the methods of the `nn_module`. When you assign an object to `self` in `initialize`, it can then be accessed by `forward`.
...
The `setup()` function emmbeds these specification into a new model object. 
We also use the `set_hparam` to specify the arguments that should be passed to the 
...
We can plot the `fiited` model to display the mean absolute error for the training and test data. 
...
allowing to represent any dataset without making assumptions on where and
how the data is organized. Usually, torch datasets also implement the 
...
There are 60,000 images in the training data and 10,000 in the test data. The images are $28\times 28$, and stored as matrix of pixels and we need to transform each one into a vector.  
...
To make this transformations we will re-define `train_ds` and `test_ds` but now passing the `transform` argument that will apply a transformation for each of
the image inputs.
...
will have 3 of them, each one descreasing the number of output units. The last,
will have 10 output units, because each unit will be associtated to a different
class and we have a 10-class classification problem.
We also defined dropout layers using `nn_dropout()` that will be used
to perform dropout regularization. And finally we define the activation
layer using `nn_relu()`.

In `forward` we define the order that these layers are called. We call them in blocks like (linear, activation, dropout) except from the last layer that we don't
...
the logits for numerical stability reasons and for memory efficiency reasons
it doesn't require the target to be one hot encoded.
...
# get's the true classes from all observations in test_ds.
...
another dense layer of size 512, which finally reaches
output layer.
...
preds_classes <- mapping[as.integer(torch_argmax(preds, dim = 2))]
preds_classes
```

## IMDb Document Classification

Now we perform document classification (Section ???) on the `IMDB` dataset, which is available as part of the `torchdatasets`
...
fitted <- model %>% 
  fit(
    # we transform the training a validation data into torch tensors
    list(
